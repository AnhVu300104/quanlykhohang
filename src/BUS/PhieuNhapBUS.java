...
public class PhieuNhapBUS {
    ...
    public ArrayList<ChiTietPhieuDTO> getChiTietPhieu_Type(int maphieunhap) {
        ...
        for (ChiTietPhieuNhapDTO item : arr) {
            ChiTietPhieuDTO dto = new ChiTietPhieuDTO();
            dto.setMaphieu(item.getMaphieu());
            dto.setMaphienbansp(item.getMaphienbansp());
    ...
    public boolean add(PhieuNhapDTO phieu, ArrayList<ChiTietPhieuNhapDTO> ctPhieu, HashMap<Integer, ArrayList<ChiTietSanPhamDTO>> chitietsanpham) {
        ...
        ArrayList<ChiTietSanPhamDTO> ctspList = convertHashMapToArray(chitietsanpham);
        check = chitietsanphamDAO.insert_mutiple(ctspList) != 0;
        return check;
    }

    public ChiTietPhieuNhapDTO findCT(ArrayList<ChiTietPhieuNhapDTO> ctphieu, int mapb) {
        for (ChiTietPhieuNhapDTO item : ctphieu) {
            if (item.getMaphienbansp() == mapb) {
                return item;
            }
        }
        return null;
    ...
    public ArrayList<PhieuNhapDTO> fillerPhieuNhap(int type, String input, int mancc, int manv, Date time_s, Date time_e, String price_minnn, String price_maxxx) {
        ...
        for (PhieuNhapDTO phieuNhap : getAllList()) {
            boolean match = false;
            switch (type) {
                case 0 -> match = Integer.toString(phieuNhap.getMaphieu()).contains(input)
                ...
                case 1 -> match = Integer.toString(phieuNhap.getMaphieu()).contains(input);
                case 2 -> match = nccBUS.getTenNhaCungCap(phieuNhap.getManhacungcap()).toLowerCase().contains(input);
                case 3 -> match = nvBUS.getNameById(phieuNhap.getManguoitao()).toLowerCase().contains(input);
            }
